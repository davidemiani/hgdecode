...
def select_classes_cnt(cnt, class_numbers):
    ...
    new_events = [ev for ev in events if
                  (ev[2] - ev[1]) in class_numbers]
...
def select_trials_cnt(cnt, inds):
    cnt = deepcopy(cnt)
    assert np.all(
        [i in np.arange(len(cnt.info['events'])) for i in inds])
    events = cnt.info['events']
    new_events = [ev for i_trial, ev in enumerate(events) if
                  i_trial in inds]
    cnt.info['events'] = np.array(new_events)
    return cnt
...
def calculate_csp(epo, classes=None, average_trial_covariance=False):
    ...
    if average_trial_covariance:
        ...
        # we need a matrix of the form (channels, observations) so we stack
        # trials and time per channel together
    ...
    # compare
    # https://github.com/bbci/bbci_public/blob/
    # c7201e4e42f873cced2e068c6cbb3780a8f8e9ec/processing/proc_csp.m#L112
    # with W := v
...
def apply_csp_var_log(epo, filters, columns):
    ...
    # 1 is t
    csp_filtered.X = np.array([
        np.log(np.var(trial, axis=1)) for trial in csp_filtered.X
    ])
    return csp_filtered
