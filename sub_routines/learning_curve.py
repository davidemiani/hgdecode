...
learning_dir = join(results_dir, 'hgdecode', learning_type, 'FBCSP_rLDA')
savings_dir = join(learning_dir, 'learning_curve_-500_4000')
touch_dir(savings_dir)

# fold stuff
n_folds_list = [2, 4, 6, 8, 10, 12]
training_ids = ['2019-01-26_18-54-54',
                '2019-01-26_20-42-32',
                '2019-01-26_23-46-51',
                '2019-01-20_05-23-10',
                '2019-01-27_03-33-19',
                '2019-01-27_10-16-16']
n_trainings = len(training_ids)
...
for subj, current_n_trials in zip(subj_str_list, n_trials_list):
    ...
    for idx, current_n_folds in enumerate(n_folds_list):
        ...
        for fold_str in [get_fold_str(x + 1) for x in range(n_folds[idx])]:
            file_path = join(learning_dir, training_ids[idx],
...
s_acc = np.std(np.array(results['m_acc']), axis=0)
